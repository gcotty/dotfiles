...
do
  ...
  vim.api.nvim_create_autocmd('FileType', {
    desc = 'Use two-space indentation for web and SQL files',
    group = indent_group,
    pattern = { 'css', 'html', 'javascript', 'javascriptreact', 'json', 'jsonc', 'scss', 'sql', 'typescript', 'typescriptreact', 'vue' },
    callback = function()
...
do
  ...
  -- VS Code-like git colours in nvim-tree. Reapply them if the colorscheme changes.
  local function set_nvim_tree_git_highlights()
    vim.api.nvim_set_hl(0, 'NvimTreeGitDirtyIcon', { fg = '#F14C4C' })
  ...
  vim.api.nvim_create_autocmd('ColorScheme', {
    group = vim.api.nvim_create_augroup('nvim-tree-git-highlights', { clear = true }),
  ...
  statusline.section_location = function() return '%2l:%-2v %L ln %p%%' end
  ...
  require('nvim-tree').setup {
    -- Reveal the active file when switching buffers. Cursor movement inside the
    -- explorer remains untouched until another file buffer is entered.
    update_focused_file = {
      enable = true,
    },
    view = {
